def delete_nth(order, max_e):
    ...
    for x in range(len(order)):
        new_order.append((order[x]))
        if new_order.count(new_order[x]) > max_e:
            new_order.pop(x)
            new_order.append("")
...
since the motif usually repeats. He isn't fond of seeing the Eiffel tower 40 times. He tells them that he will only 
sit for the session if they show the same motif at most N times. Luckily, Alice and Bob are able to encode the motif 
as a number. Can you help them to remove numbers such that their list contains each number only up to N times, 
